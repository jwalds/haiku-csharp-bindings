# haiku-csharp-bindings: design details

This file holds the full design rationale for `haiku-csharp-bindings` --
see [README.md](README.md) for the short version, build instructions, and
license. Nothing here is required reading to build or run the bindings,
but it's the first place to look before extending them: why a hand-written
shim over generating one, prior art, the current API surface class by
class, and a deep-dive section per kit/widget covering every fact verified
on real Haiku hardware before being relied on. See
[`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) for open bugs found on real hardware
